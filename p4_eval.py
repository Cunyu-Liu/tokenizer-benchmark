...
# BLT arms are scored on a deterministic homology-stratified test subsample
# (exact causal next_base is O(L^2)/seq; full 108K test is prohibitive). The
# subsample is generated by p4_subsample.py. Flat arms score the full test split.
BLT_SUBSAMPLE_PARQUET = "/mnt/cunyuliu/tokenizer-benchmark/runs/blt_test_subsample.parquet"
...
def _blt_seqs(subsample: str, n: int | None = None) -> list[str]:
    """BLT arms read the fixed homology-stratified test subsample parquet."""
    out = []
...
def main() -> None:
    ...
    ap.add_argument("--subsample", default=None,
                    help="homology-stratified test subsample parquet (BLT arms; "
                         "defaults to BLT_SUBSAMPLE_PARQUET)")
    ...
    if cfg.arm.backbone == "blt":
        ...
        seqs = _blt_seqs(args.subsample or BLT_SUBSAMPLE_PARQUET, args.n)
    else:
